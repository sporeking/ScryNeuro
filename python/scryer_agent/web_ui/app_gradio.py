...
_DEFAULT_TOOL_INFO = "默认仅启用低风险只读工具；`shell_exec` 与 `write_file` 属于高风险能力，需要手动勾选。"


def _default_enabled_tool_names(tool_rows: list[dict[str, Any]]) -> list[str]:
    out: list[str] = []
    for row in tool_rows:
        ...
        if not isinstance(metadata, dict):
            continue
        if bool(metadata.get("default_enabled", False)):
...
def _refresh_catalogs() -> tuple[list[str], list[str], list[str], list[str], str]:
    ...
    tool_rows = list_tools()
    tool_names = [str(x.get("name", "")) for x in tool_rows if x.get("name")]
...
def build_app() -> Any:
    ...
    default_tools = _default_enabled_tool_names(list_tools())
    ...
    with gr.Blocks(title="ScryNeuro Agent Playground") as demo:
        ...
        with gr.Row():
            with gr.Column(scale=1):
                ...
                gr.Markdown(_DEFAULT_TOOL_INFO)
...
def launch() -> Any:
    app = build_app()
    return app.launch(server_name="127.0.0.1", server_port=7860)
