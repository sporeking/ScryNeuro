...
from .tool_runtime import _tool_registry
from .config import get_profile, list_profiles, resolve_profile
...
class AgentManager:
    ...
    def discover_skills(
        self, skills_dir: str = "python/scryer_agent/skills"
    ) -> list[dict[str, Any]]:
        base = _resolve_skills_dir(skills_dir)
        if not base.is_dir():
            return []
...
def agent_discover_skills(kwargs: Optional[dict] = None) -> list[dict[str, Any]]:
    if kwargs is None:
        kwargs = {}
    skills_dir = str(kwargs.get("skills_dir", "python/scryer_agent/skills"))
